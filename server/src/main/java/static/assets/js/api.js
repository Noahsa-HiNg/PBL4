...
function getAuthHeaders() {
    ...
    if (!token) {
        ...
        const error = new Error("Bạn cần đăng nhập để thực hiện việc này.");
        error.status = 401; 
        throw error;
    }
    return {
...
async function handleResponseError(response) {
    ...
    try {
        ...
        if (serverErrorData.message && typeof serverErrorData.message === 'string' && serverErrorData.message.trim() !== "") {
            userMessage = serverErrorData.message;
        }
    } catch (e) {

        console.warn("Không thể parse JSON lỗi từ server, hoặc body rỗng.", response.status);
    }

    if (userMessage === "") {
        switch (response.status) {
            case 400:
                userMessage = "Dữ liệu gửi đi không hợp lệ. Vui lòng kiểm tra lại thông tin.";
                break;
            case 401:
                userMessage = "Sai tên đăng nhập hoặc mật khẩu.";
                break;
            case 403:
            ...
            case 502:
            case 503:
                userMessage = "Máy chủ đang gặp sự cố. Vui lòng thử lại sau giây lát.";
                break;
            default:
                userMessage = `Yêu cầu thất bại. (Mã lỗi: ${response.status})`;
        }
    }
    
    // 4. Ném lỗi mới với thông báo đã được dịch
    const error = new Error(userMessage);
    error.status = response.status;
    error.serverData = serverErrorData; // Gắn thêm data gốc (nếu UI cần)
    throw error;
}
/**
